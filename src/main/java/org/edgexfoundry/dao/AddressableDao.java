...
/**
 * Utility class around AddressableRepository
 * 
 */
@Component
public class AddressableDao {
	...
	@Autowired
	private DeviceManagerRepository deviceManagerRepos;
	...
	public boolean isAddressableAssociatedToDevice(Addressable addressable) {
		return !deviceRepos.findByAddressable(addressable).isEmpty()
				|| !deviceManagerRepos.findByAddressable(addressable).isEmpty();
	}
	...
	public List<Asset> getOwningServices(Addressable addressable) {
		return getAssociatedDevices(addressable).stream().map(Device::getService).collect(Collectors.toList());
	}

	private List<Device> getAssociatedDevices(Addressable addressable) {
		List<Device> assocDevices = deviceRepos.findByAddressable(addressable);
		assocDevices.addAll(deviceManagerRepos.findByAddressable(addressable));
		return assocDevices;
	}
