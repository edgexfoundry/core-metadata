...
public class CommandController {
	...
	// Logger.getLogger(CommandController.class);
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = org.edgexfoundry.support.logging.client.EdgeXLoggerFactory
			.getEdgeXLogger(CommandController.class);
	...
	/**
	 * Fetch a specific command by database generated id. May return null if no
	 * commands with the id is found. Returns ServiceException (HTTP 503) for
	...
	/**
	 * Return all command objects. Returns ServiceException (HTTP 503) for
	...
	/**
	 * Return Command object with given name. Name is not unique for all of EdgeX
	 * but is unique per any associated Device Profile. Returns ServiceException
	...
	/**
	 * Add a new Command. Returns ServiceException (HTTP 503) for unknown or
	 ...
	 * unanticipated issues. NotFoundException (HTTP 404) if no Command is found
	 * with the provided id. DataValidationException (HTTP 409) if the name is
	 * updated and it is not unique for the Device Profile.
	 * 
	 * @param object
	 *            holding the database generated identifier and new values for
	 *            the Command
	...
	/**
	 * Remove the Command designated by database generated id. ServiceException
	 * (HTTP 503) for unknown or unanticipated issues. NotFoundException (HTTP
	 * 404) if no Command is found with the provided id. DataValidationException
	 * (HTTP 409) if the Command is still associated to a Device Profile.
	 * 
	 * @param database
	 *            generated id for the Command
	...
	public boolean delete(@PathVariable String id) {
		try {
			...
			if (profileDao.getAssociatedProfilesForCommand(command).isEmpty()) {
				...
				throw new DataValidationException("Command is still referenced by DeviceProfiles - cannot be deleted");
			}
		} catch (NotFoundException nE) {
			throw nE;
		} catch (DataValidationException dE) {
