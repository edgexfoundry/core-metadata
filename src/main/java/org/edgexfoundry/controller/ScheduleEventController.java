...
public class ScheduleEventController {
	...
	// Logger.getLogger(ScheduleEventController.class);
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = org.edgexfoundry.support.logging.client.EdgeXLoggerFactory
			.getEdgeXLogger(ScheduleEventController.class);
	...
	/**
	 * Fetch a specific ScheduleEvent by database generated id. May return null
	 * if no schedule event with the id is found. Returns ServiceException (HTTP
	 * 503) for unknown or unanticipated issues. Returns NotFoundException (HTTP
	 * 404) if a ScheduleEvent cannot be found by the id.
	 * 
	 * @param String
	 *            database generated id for the schedule event
	 * 
	...
	/**
	 * Return all schedule events sorted by id. Returns ServiceException (HTTP
	...
	/**
	 * Return ScheduleEvents matching given name (schedule names should be
	 * unique). May be null if no schedule events matches on the name provided.
	 * Returns ServiceException (HTTP 503) for unknown or unanticipated
	 * issues.Returns NotFoundException (HTTP 404) if a ScheduleEvent cannot be
	 * found by the name.
	 * 
	 * @param name
	...
	/**
	 * Find all schedule events associated to the Addressable with the specified
	 * addressable database generated identifier. List may be empty if no
	 * schedule events match. Returns ServiceException (HTTP 503) for unknown or
	...
	public List<ScheduleEvent> scheduleEventsForAddressable(@PathVariable String addressableId) {
		try {
			...
			logger.error("Error getting Schedule Event:  " + e.getMessage());
			throw new ServiceException(e);
		}
	}
	...
	/**
	 * Find all schedule events associated to the Addressable with the specified
	 * addressable name. List may be empty if no schedule events match. Returns
	...
	public List<ScheduleEvent> scheduleEventsForAddressableByName(@PathVariable String addressablename) {
		try {
			Addressable addressable = addressableDao.getByName(addressablename);
			if (addressable == null) {
				logger.error("Request for Schedule Events by non-existent addressable:  " + addressablename);
	...
	/**
	 * Find all schedule events associated to the service with the specified
	 * service name. List may be empty if no service names match. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 * NotFoundException (HTTP 404) if no Schedule Events match on the name
	 * provided.
	 * 
	 * @param addressable
	 *            name - addressable's name
	...
	public List<ScheduleEvent> scheduleEventsForServiceByName(@PathVariable String servicename) {
		try {
			if (isServiceNameValid(servicename)) {
				...
				logger.error("Service with name: " + servicename + " was not found.");
				// TODO: throw new NotFoundException(Service.class.toString(),
				// servicename);
	...
	/**
	 * Add a new ScheduleEvent - name must be unique. Returns ServiceException
	 * (HTTP 503) for unknown or unanticipated issues. NotFoundException (HTTP
	 * 404) if the event's associated schedule is not found (referenced by
	 * name). DataValidationException (HTTP 409) if the schedule was not
	 * provided.
	 * 
	 * @param Schedule
	 *            object
	...
	public String add(@RequestBody ScheduleEvent scheduleEvent) {
		...
		if (scheduleEvent.getSchedule() == null)
			throw new ServiceException(new DataValidationException("No schedule provided for schedule event"));
		try {
			if (isScheduleNameValid(scheduleEvent.getSchedule())) {
				...
				logger.error("Schedule with name: " + scheduleEvent.getSchedule() + " was not found.");
	...
	private void attachAssociated(ScheduleEvent event) {
		...
		if (addressable == null)
			throw new DataValidationException("A schedule event must be associated to a known addressable.");
	 ...
	 * unanticipated issues. DataValidationException (HTTP 409) if an attempt to
	 * change the name is made when the schedule event is still being referenced
	 * by device reports. NotFoundException (HTTP 404) if no schedule is found
	...
	public boolean update(@RequestBody ScheduleEvent scheduleEvent2) {
		try {
			...
			if (scheduleEvent == null) {
				logger.error("Request to update with non-existent or unidentified ScheduleEvent (id/name):  "
	...
	private void updateScheduleEvent(ScheduleEvent from, ScheduleEvent to) {
		...
		if (from.getService() != null)
			if (isServiceNameValid(from.getService())) {
				...
				logger.error("Service with name: " + from.getService() + " was not found.");
				// TODO: throw new NotFoundException(Service.class.toString(),
				// from.getServiceName());
		...
		if (from.getSchedule() != null)
			if (isScheduleNameValid(from.getSchedule()))
				...
				logger.error("Schedule with name: " + from.getSchedule() + " was not found.");
		...
		if (from.getName() != null) {
			if (dao.isScheduleEventAssociatedToDeviceReport(to)) {
				logger.error("Data integrity issue. ScheduleEvent with name: " + from.getName()
						+ "is still referenced by existing DeviceReport.");
				throw new DataValidationException("Data integrity issue. ScheduleEvent with name: " + from.getName()
						+ "is still referenced by existing DeviceReport.");
			} else
	...
	/**
	 * Remove the ScheduleEvent designated by database generated id.
	 * ServiceException (HTTP 503) for unknown or unanticipated issues.
	 * NotFoundException (HTTP 404) if no ScheduleEvent is found with the
	 * provided id. DataValidationException (HTTP 409) if an attempt to delete a
	 * schedule event still being referenced by device reports.
	 * 
	 * @param database
	 *            generated id for the ScheduleEvent
	...
	/**
	 * Remove the ScheduleEvent designated by name. ServiceException (HTTP 503)
	 * for unknown or unanticipated issues. NotFoundException (HTTP 404) if no
	 * ScheduleEvent is found with the provided name. DataValidationException
	 * (HTTP 409) if an attempt to delete a schedule event still being
	 * referenced by device reports.
	 * 
	 * @param name
	...
	private boolean deleteScheduleEvent(ScheduleEvent scheduleEvent) {
		if (dao.isScheduleEventAssociatedToDeviceReport(scheduleEvent)) {
			logger.error("Data integrity issue. ScheduleEvent with id: " + scheduleEvent.getId()
					+ "is still referenced by existing DeviceReport.");
			throw new DataValidationException("Data integrity issue. ScheduleEvent with id: " + scheduleEvent.getId()
					+ "is still referenced by existing DeviceReport.");
		}
	...
	private boolean isServiceNameValid(String name) {
		...
		// return serviceRepos.findByName(name) != null;
		return true;
	}

	private void notifyAssociates(ScheduleEvent event, Action action) {
		callback.callback(dao.getAffectedService(event), event.getId(), action, ActionType.SCHEDULEEVENT);
	}
