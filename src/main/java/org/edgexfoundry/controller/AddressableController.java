...
public class AddressableController {
	...
	// Logger.getLogger(AddressableController.class);
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = org.edgexfoundry.support.logging.client.EdgeXLoggerFactory
			.getEdgeXLogger(AddressableController.class);
	...
	/**
	 * Fetch a specific addressable by database generated id. May return null if
	 * no addressable matches on id. Returns ServiceException (HTTP 503) for
	 * unknown or unanticipated issues. Returns NotFoundException (HTTP 404) if
	 * not found by id.
	 * 
	 * @param String
	 *            addressable database generated id
	 * @return Addressable
	 */
	...
	/**
	 * Return all addressable objects sorted by database generated id. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 * LimitExceededException (HTTP 413) if the number returned exceeds the max
	 * limit.
	 * 
	...
	/**
	 * Return Addressable with matching name (name should be unique). May be
	 * null if none match. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues.  Returns NotFoundException (HTTP 404) if
	 * not found by name.
	 * 
	 * @param name
	...
	public Addressable addressableForName(@PathVariable String name) {
		try {
			Addressable a = repos.findByName(name);
			if (a==null)
	...
	/**
	 * Return Addressable objects with given address. List may be empty if none
	 * are associated to the address. Returns ServiceException (HTTP 503) for
	 * unknown or unanticipated issues.
	 * 
	 * @param address
	 *            string (like a URL address)
	...
	/**
	 * Return Addressable objects with given port. List may be empty if none are
	 * associated to the port. Returns ServiceException (HTTP 503) for unknown
	...
	/**
	 * Return Addressable objects with given topic. List may be empty if none
	 * are associated to the topic. Returns ServiceException (HTTP 503) for
	 * unknown or unanticipated issues.
	 * 
	 * @param topic
	...
	/**
	 * Return Addressable objects with given publisher. List may be empty if
	 * none are associated to the publisher. Returns ServiceException (HTTP 503)
	...
	/**
	 * Add a new Addressable - name must be unique. Returns ServiceException
	...
	/**
	 * Update the Addressable identified by the id or name in the object
	 ...
	 * addressable with the provided id is found.
	 * 
	 * @param Addressable
	...
	private void updateAddressable(Addressable from, Addressable to) {
		...
		if (from.getName() != null && !from.getName().equals(to.getName())) {
			if ((dao.isAddressableAssociatedToDevice(to) || dao.isAddressableAssociatedToDeviceService(to))) {
				logger.error("Data integrity issue. Addressable with name: " + from.getName()
						+ "is still referenced by existing devices or device services.");
				throw new DataValidationException("Data integrity issue. Addressable with name: " + from.getName()
						+ "is still referenced by existing devices or device services.");
			} else
	...
	/**
	 * Remove the Addressable designated by the database generated id for the
	 * Addressable. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if no
	 * addressable with the provided id is found.
	 * 
	 * @param database
	...
	/**
	 * Remove the Addressable designated by unique name identifier. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 * NotFoundException (HTTP 404) if no addressable with the provided name is
	 * found.
	 * 
	 * @param unique
	...
	private boolean deleteAddressable(Addressable addressable) {
		if (dao.isAddressableAssociatedToDevice(addressable)
			...
			logger.error("Data integrity issue. Addressable with id: " + addressable.getId()
					+ "is still referenced by existing devices or device services.");
			throw new DataValidationException("Data integrity issue. Addressable with id: " + addressable.getId()
					+ "is still referenced by existing devices or device services.");
		}
	...
	private void notifyAssociates(Addressable addressable, Action action) {
		callback.callback(dao.getOwningServices(addressable), addressable.getId(), action, ActionType.ADDRESSABLE);
	}
