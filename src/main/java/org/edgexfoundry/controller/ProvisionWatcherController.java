...
public class ProvisionWatcherController {

	// private static final Logger logger =
	// Logger.getLogger(ProvisionWatcherController.class);
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = org.edgexfoundry.support.logging.client.EdgeXLoggerFactory
			.getEdgeXLogger(ProvisionWatcherController.class);
	...
	/**
	 * Fetch a specific Provision Watcher by database generated id. May return
	 * null if no ProvisionWatcher matches on id. Returns ServiceException (HTTP
	 * 503) for unknown or unanticipated issues. Return NotFoundException (HTTP
	 * 404) if no ProvisionWatcher is found by the id provided;
	 * 
	 * @param String
	 *            ProvisionWatcher database generated id
	 * @return ProvisionWatcher
	 */
	@RequestMapping(value = "/{id}", method = RequestMethod.GET)
	public ProvisionWatcher watcher(@PathVariable String id) {
		try {
			ProvisionWatcher w = repos.findOne(id);
			if (w == null)
			...
			logger.error("Error getting provision watcher:  " + e.getMessage());
			throw new ServiceException(e);
		}
	}

	/**
	 * Return all provision watcher objects sorted by database generated id.
	 * Returns ServiceException (HTTP 503) for unknown or unanticipated issues.
	 * Returns LimitExceededException (HTTP 413) if the number returned exceeds
	 * the max limit.
	 * 
	...
	/**
	 * Return ProvisionWatcher with matching name (name should be unique). May
	 * be null if none match. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Return NotFoundException (HTTP 404) if no
	 * ProvisionWatcher is found by the name provided;
	 * 
	 * @param name
	...
	public ProvisionWatcher watcherForName(@PathVariable String name) {
		try {
			ProvisionWatcher w = repos.findByName(name);
			if (w == null)
	...
	/**
	 * Find all provision watchers associated to the DeviceProfile with the
	 * specified profile database generated identifier. List may be empty if no
	 * provision watchers match. Returns ServiceException (HTTP 503) for unknown
	...
	/**
	 * Find all provision watchers associated to the DeviceProfile with the
	 * specified profile name. List may be empty if no provision watchers match.
	 * Returns ServiceException (HTTP 503) for unknown or unanticipated issues.
	 * Returns NotFoundException (HTTP 404) if no DeviceProfile match on the
	 * name provided.
	 * 
	 * @param profile
	 *            name - provision watcher profile's name
	...
	/**
	 * Find the provision watchers associated to the DeviceService with the
	 * specified service database generated identifier. List may be empty if no
	 * provision watchers match. Returns ServiceException (HTTP 503) for unknown
	 * or unanticipated issues. Returns NotFoundException (HTTP 404) if no
	 * DeviceService match on the id provided.
	 * 
	 * @param service
	 *            id - device service's database generated identifier
	...
	/**
	 * Find provision watchers associated to the DeviceService with the
	 * specified service name. List may be empty if no provision watchers match.
	 * Returns ServiceException (HTTP 503) for unknown or unanticipated issues.
	 * Returns NotFoundException (HTTP 404) if no DeviceService match on the
	 * name provided.
	 * 
	 * @param service
	 *            name - provision watcher service's name
	 * @return ProvisionWatcher associated to the device service
	 */
	...
	/**
	 * Find ProvisionWatchers by an identifier key/value pair. Returns
	...
	public List<ProvisionWatcher> watchersForIdentifier(@PathVariable String key, @PathVariable String value) {
		try {
	...
	/**
	 * Add a new ProvisionWatcher - name must be unique. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues.Returns
	 * DataValidationException (HTTP 409) if an associated object (Profile,
	 * Service) cannot be found with the id or name provided.
	 * 
	 * @param ProvisionWatcher
	 *            to add
	...
	private void attachAssociated(ProvisionWatcher watcher) {
		...
		if (service == null)
			throw new DataValidationException("A provision watcher must be associated to a known device service.");
	 ...
	 * purposes. Watcher's service & profile cannot be updated. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 * NotFoundException (HTTP 404) if no provision watcher with the provided id
	 * is found.
	 * 
	 * @param ProvisionWatcher
	...
	public boolean update(@RequestBody ProvisionWatcher watcher2) {
		try {
			...
			if (watcher == null) {
				logger.error("Request to update with non-existent or unidentified provision watcher (id/name):  "
	...
	/**
	 * Remove the ProvisionWatcher designated by the database generated id for
	 * the ProvisoinWatcher. Returns ServiceException (HTTP 503) for unknown or
	...
	/**
	 * Remove the ProvisionWatcher designated by unique name identifier. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 * NotFoundException (HTTP 404) if no provision watcher with the provided
	 * name is found.
	 * 
	 * @param unique
	 *            name of the ProvisionWatcher
	 * @return boolean indicating success of the remove operation
	 */
