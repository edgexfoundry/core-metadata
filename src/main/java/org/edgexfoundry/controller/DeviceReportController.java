...
public class DeviceReportController {
	...
	// Logger.getLogger(DeviceReportController.class);
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = org.edgexfoundry.support.logging.client.EdgeXLoggerFactory
			.getEdgeXLogger(DeviceReportController.class);
	...
	/**
	 * Fetch a specific DeviceReport by database generated id. May return null
	 * if no report with the id is found. Returns ServiceException (HTTP 503)
	 * for unknown or unanticipated issues. Returns NotFoundException (HTTP 404)
	 * if the report cannot be found by id;
	 * 
	 * @param String
	 *            database generated id for the report
	 * 
	...
	public DeviceReport deviceReport(@PathVariable String id) {
		try {
			DeviceReport r = repos.findOne(id);
			if (r == null)
	...
	/**
	 * Return all device reports sorted by id. Returns ServiceException (HTTP
	...
	/**
	 * Return DeviceReport matching given name (device report names should be
	 * unique). May be null if no report matches on the name provided. Returns
	...
	public DeviceReport deviceReportForName(@PathVariable String name) {
		try {
			DeviceReport r = repos.findByName(name);
			if (r== null)
	...
	/**
	 * Return a list of value descriptor names - this list is the union of all
	 * value descriptor names from all the device reports associated to the
	 * named device. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues.
	 * 
	 * @param devicename
	 *            - the unique name of the device that has device reports
	...
	public List<String> associatedValueDescriptors(@PathVariable String devicename) {
		try {
			...
			logger.error(
					"Error getting value descriptors associted with device reports for a device:  " + e.getMessage());
			throw new ServiceException(e);
		}
	}
	...
	/**
	 * Add a new DeviceReport - name must be unique. Referenced objects (device,
	 * schedule event) are all referenced in the new DeviceReport by name and
	 * must already be persisted. Returns ServiceException (HTTP 503) for
	 * unknown or unanticipated issues. NotFoundException (HTTP 404) if any
	 * referenced object cannot be found by its provided name.
	 * 
	 * @param DeviceReport
	 *            object
	 ...
	 * unanticipated issues. NotFoundException (HTTP 404) if any referenced
	...
	private void updateDeviceReport(DeviceReport from, DeviceReport to) {
		...
		if (from.getName() != null) // device report name is not referenced by
	...
	/**
	 * Remove the DevicReport designated by database generated id.
	 * ServiceException (HTTP 503) for unknown or unanticipated issues.
	 * NotFoundException (HTTP 404) if no DeviceReport is found with the
	 * provided id.
	 * 
	 * @param database
	 *            generated id for the DeviceReport
	...
	/**
	 * Remove the DevicReport designated by name. ServiceException (HTTP 503)
	 * for unknown or unanticipated issues. NotFoundException (HTTP 404) if no
	 * DeviceReport is found with the provided name.
	 * 
	 * @param name
	 *            for the DeviceReport
	 * @return boolean indicating success of the remove operation
	 */
