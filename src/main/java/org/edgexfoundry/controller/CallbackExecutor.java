...
public class CallbackExecutor {
	...
	//private static final Logger logger = Logger.getLogger(CallbackExecutor.class);
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = 
			org.edgexfoundry.support.logging.client.EdgeXLoggerFactory.getEdgeXLogger(CallbackExecutor.class);
	...
	public void callback(Asset asset, final String id, final Action action, final ActionType type) {
		...
		try {
			if (url != null) {
				...
				logger.info("Call back device service @:  " + url + " with:  " + body + " received status code:  "
						+ returnCode);
			} else
