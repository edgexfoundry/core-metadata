...
public class DeviceController {
	...
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = org.edgexfoundry.support.logging.client.EdgeXLoggerFactory
			.getEdgeXLogger(DeviceController.class);
	...
	/**
	 * Fetch a specific device by database generated id. May return null if no
	 * device with the id is found. Returns ServiceException (HTTP 503) for
	 * unknown or unanticipated issues. Returns NotFoundException (HTTP 404) if
	 * device not found by id.
	 * 
	 * @param String
	 *            database generated id for the device
	 * 
	...
	public Device device(@PathVariable String id) {
		try {
			...
			logger.error("Error getting device:  " + e.getMessage());
			throw new ServiceException(e);
		}
	}

	/**
	 * Return all devices sorted by id. Returns ServiceException (HTTP 503) for
	 * unknown or unanticipated issues. Returns LimitExceededException (HTTP
	...
	/**
	 * Return Device matching given name (device names should be unique). May be
	 * null if no device matches on the name provided. Returns ServiceException
	 * (HTTP 503) for unknown or unanticipated issues. Returns NotFoundException
	 * (HTTP 404) if device not found by name.
	 * 
	 * @param name
	...
	public Device deviceForName(@PathVariable String name) {
		try {
			Device d = repos.findByName(name);
			if (d == null)
	...
	/**
	 * Find all Devices having at least one label matching the label provided.
	 * List may be empty if no device match. Returns ServiceException (HTTP 503)
	...
	/**
	 * Find all devices associated to the DeviceService with the specified
	 * DeviceService database generated identifier. List may be empty if no
	 * device match. Returns ServiceException (HTTP 503) for unknown or
	 ...
	 * no device match. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if no
	...
	/**
	 * Find all devices associated to the DeviceProfile with the specified
	 * profile database generated identifier. List may be empty if no device
	...
	/**
	 * Find all devices associated to the DeviceProfile with the specified
	 * profile name. List may be empty if no device match. Returns
	...
	/**
	 * Find all devices associated to the Addressable with the specified
	 * addressable database generated identifier. List may be empty if no device
	 * match. Returns ServiceException (HTTP 503) for unknown or unanticipated
	 * issues. Returns NotFoundException (HTTP 404) if no Addressable match on
	 * the id provided.
	 * 
	 * @param addressable
	...
	/**
	 * Find all devices associated to the Addressable with the specified
	 * addressable name. List may be empty if no device match. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	...
	/**
	 * Add a new Device - name must be unique. Embedded objects (device,
	 * service, profile, addressable) are all referenced in the new Device
	 * object by id or name to associated objects. All other data in the
	 * embedded objects will be ignored. Returns ServiceException (HTTP 503) for
	 * unknown or unanticipated issues. Returns DataValidationException (HTTP
	 * 409) if an associated object (Addressable, Profile, Service) cannot be
	 * found with the id or name provided.
	 * 
	 * @param Device
	 *            object
	...
	/**
	 * Update the last connected time of the device by database generated
	 ...
	 * 
	 * @param id
	 *            - database generated identifier for the device
	 * @param time
	 ...
	 *            - new last connected time in milliseconds
	 * @param notify
	 *            - boolean indicating whether callback should be made to device
	 *            service to inform of update
	 * @return boolean indicating success of update
	 */
	@RequestMapping(value = "/{id}/lastconnected/{time}/{notify}", method = RequestMethod.PUT)
	public boolean updateLastConnected(@PathVariable String id, @PathVariable long time, @PathVariable boolean notify) {
		try {
	...
	/**
	 * Update the last connected time of the device by unique name of the
	 * device. Returns ServiceException (HTTP 503) for unknown or unanticipated
	 ...
	 * @return boolean indicating success of update
	 */
	@RequestMapping(value = "/name/{name:.+}/lastconnected/{time}/{notify}", method = RequestMethod.PUT)
	public boolean updateLastConnectedByName(@PathVariable String name, @PathVariable long time,
	...
	/**
	 * Update the last reported time of the device by database generated
	 * identifier. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if the device
	 * cannot be found by the identifier provided.
	 * 
	...
	public boolean updateLastReported(@PathVariable String id, @PathVariable long time, @PathVariable boolean notify) {
		try {
			Device device = repos.findOne(id);
			if (device == null) {
	...
	/**
	 * Update the last reported time of the device by unique name of the device.
	 ...
	 * @return boolean indicating success of update
	 */
	@RequestMapping(value = "/name/{name:.+}/lastreported/{time}/{notify}", method = RequestMethod.PUT)
	public boolean updateLastReportedByName(@PathVariable String name, @PathVariable long time,
			@PathVariable boolean notify) {
		try {
			Device device = repos.findByName(name);
	...
	/**
	 * Update the op state of the device by database generated identifier.
	 * Returns ServiceException (HTTP 503) for unknown or unanticipated issues.
	 * Returns NotFoundException (HTTP 404) if the device cannot be found by the
	 ...
	 *            - database generated identifier for the device
	 * @param opState
	 *            - new op state for the device (either enabled or disabled)
	...
	/**
	 * Update the op status time of the device by unique name of the device.
	 * Returns ServiceException (HTTP 503) for unknown or unanticipated issues.
	 * Returns NotFoundException (HTTP 404) if the device cannot be found by the
	 * name provided. Returns DataValidationException (HTTP 409) if the proposed
	 * new state is null;
	 * 
	 * @param name
	 *            - device name
	...
	/**
	 * Update the admin state of the device by database generated identifier.
	 * Returns ServiceException (HTTP 503) for unknown or unanticipated issues.
	 * Returns NotFoundException (HTTP 404) if the device cannot be found by the
	 * identifier provided. Returns DataValidationException (HTTP 409) if the
	 * proposed new state is null;
	 ...
	 *            - database generated identifier for the device
	 * @param adminstate
	 *            - new admin state for the device (either locked or unlocked)
	 * @return - boolean indicating success of the operation
	 */
	...
	/**
	 * Update the admin state of the device by device name. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 * NotFoundException (HTTP 404) if the device cannot be found by the
	 * identifier provided. Returns DataValidationException (HTTP 409) if the
	 * proposed new state is null;
	 * 
	 * @param name
	 ...
	 * provided. Id is used first, name is used second for identification
	 ...
	 * unknown or unanticipated issues. Returns NotFoundException (HTTP 404) if
	 * the device cannot be found by the identifier provided
	 * 
	 * @param device2
	 *            - object holding the identifier and new values for the Device
	...
	public boolean update(@RequestBody Device device2) {
		try {
			...
			if (device == null) {
				logger.error("Request to update with non-existent or unidentified device (id/name):  " + device2.getId()
						+ "/" + device2.getName());
	...
	/**
	 * Remove the Device designated by database generated id. This does not
	 * remove associated objects (addressable, service, profile, etc.). Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 * NotFoundException (HTTP 404) if the device cannot be found by the
	 * identifier provided.
	 * 
	 * @param database
	 *            generated id for the device
	...
	/**
	 * Remove the Device designated by unique name. This does not remove
	...
	private void postNotification(String name, Action action) {
		if (notifyDeviceChanges) {
			...
			n.setSlug(notificationSlug + System.currentTimeMillis());
			n.setContent(notificationContent + name + "-" + action);
	...
	private void postNotification(Notification notification) {
		try {
			...
			logger.debug("Notification send on device modification failed for:" + notification.getSlug() + "; " + e);
		}
	}
}
