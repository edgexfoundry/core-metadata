...
public class ScheduleController {
	...
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = org.edgexfoundry.support.logging.client.EdgeXLoggerFactory
			.getEdgeXLogger(ScheduleController.class);
	...
	/**
	 * Fetch a specific Schedule by database generated id. May return null if no
	 * schedule with the id is found. Returns ServiceException (HTTP 503) for
	 * unknown or unanticipated issues. Returns NotFoundException (HTTP 404)
	 * when no schedule is found by the id provided.
	 * 
	 * @param String
	 *            database generated id for the schedule
	 * 
	...
	/**
	 * Return all schedules sorted by id. Returns ServiceException (HTTP 503)
	 * for unknown or unanticipated issues. Returns LimitExceededException (HTTP
	...
	/**
	 * Return Schedule matching given name (schedule names should be unique).
	 * May be null if no schedule matches on the name provided. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 * NotFoundException (HTTP 404) when no schedule is found by the name
	 * provided.
	 * 
	 * @param name
	...
	public Schedule scheduleForName(@PathVariable String name) {
		try {
			Schedule s = repos.findByName(name);
			if (s == null)
	...
	/**
	 * Add a new Schedule - name must be unique. Returns ServiceException (HTTP
	 * 503) for unknown or unanticipated issues. DataValidationException (HTTP
	 * 409) if any the cron expression string is not properly formatted.
	 * 
	 * @param Schedule
	 *            object
	...
	/**
	 * Update the Schedule identified by the id or name in the object provided.
	 ...
	 * DataValidationException (HTTP // * 409) if any the cron expression string
	 * is not properly formatted. NotFoundException (HTTP 404) if no schedule is
	 * found for the id.
	 * 
	 * @param schedule2
	...
	/**
	 * Remove the Schedule designated by database generated id. ServiceException
	 * (HTTP 503) for unknown or unanticipated issues. NotFoundException (HTTP
	 * 404) if no Schedule is found with the provided id.
	 * 
	 * @param database
	 *            generated id for the Schedule
	...
	/**
	 * Remove the Schedule designated by name. ServiceException (HTTP 503) for
	 * unknown or unanticipated issues. NotFoundException (HTTP 404) if no
	 * Schedule is found with the provided name.
	 * 
	 * @param name
	 *            for the Schedule
	 * @return boolean indicating success of the remove operation
	...
	private boolean deleteSchedule(Schedule schedule) {
		if (dao.isScheduleAssociatedToScheduleEvent(schedule)) {
			...
			throw new DataValidationException("Data integrity issue. Schedule with id: " + schedule.getId()
					+ "is still referenced by existing ScheduleEvents.");
		}
	...
	private void notifyAssociates(Schedule schedule, Action action) {
		callback.callback(dao.getAffectedServices(schedule), schedule.getId(), action, ActionType.SCHEDULE);
	}
}
