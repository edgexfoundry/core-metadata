...
public class DeviceServiceController {
	...
	// Logger.getLogger(DeviceServiceController.class);
	private final static org.edgexfoundry.support.logging.client.EdgeXLogger logger = org.edgexfoundry.support.logging.client.EdgeXLoggerFactory
			.getEdgeXLogger(DeviceServiceController.class);
	...
	/**
	 * Fetch a specific device service by database generated id. May return null
	 * if no service with the id is found. Returns ServiceException (HTTP 503)
	 * for unknown or unanticipated issues. Returns NotFoundException (HTTP 404)
	 * if the device service is not found by the id provided.
	 * 
	 * @param String
	 *            database generated id for the service
	 * 
	...
	public DeviceService deviceService(@PathVariable String id) {
		try {
			DeviceService s = repos.findOne(id);
			if (s == null)
			...
			logger.error("Error getting service:  " + e.getMessage());
			throw new ServiceException(e);
		}
	}

	/**
	 * Return the DeviceService matching given name (service names should be
	 * unique). May be null if no services matches on the name provided. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 * NotFoundException (HTTP 404) if the device service is not found by the
	 * name provided.
	 * 
	 * @param name
	...
	public DeviceService deviceServiceForName(@PathVariable String name) {
		try {
			DeviceService s = repos.findByName(name);
			if (s == null)
	...
	/**
	 * Return all device services sorted by id. Returns ServiceException (HTTP
	 * 503) for unknown or unanticipated issues. Returns LimitExceededException
	...
	/**
	 * Find all device servicess associated to the Addressable with the
	 * specified addressable database generated identifier. List may be empty if
	 * no device service match. Returns ServiceException (HTTP 503) for unknown
	 * or unanticipated issues. Returns NotFoundException (HTTP 404) if no
	...
	public List<DeviceService> deviceServicesForAddressable(@PathVariable String addressableId) {
		try {
			...
			logger.error("Error getting device service:  " + e.getMessage());
			throw new ServiceException(e);
		}
	}

	/**
	 * Find all device serices associated to the Addressable with the specified
	 * addressable name. List may be empty if no device services match. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	...
	public List<DeviceService> deviceServicesForAddressableByName(@PathVariable String addressablename) {
		try {
			Addressable addressable = addressableDao.getByName(addressablename);
			if (addressable == null) {
				logger.error("Request for device services by non-existent addressable:  " + addressablename);
	...
	/**
	 * Find all DeviceServices having at least one label matching the label
	 * provided. List may be empty if no device services match. Returns
	...
	/**
	 * Return a set (set versus list to insure element uniqueness) of
	 * addressables that are associated to the devices of a device service. The
	 * device service is identified by id. Returns NotFoundException (HTTP 404)
	 * if the device service is not found by id. Returns ServiceException (HTTP
	 * 503) for unknown or unanticipated issues.
	 * 
	 * @param id
	 *            - database provided id for the device service
	 * @return - set of unique addressable objects associated to devices of the
	...
	public Set<Addressable> addressablesForAssociatedDevices(@PathVariable String id) {
		try {
			...
			if (service == null)
				throw new NotFoundException(DeviceService.class.toString(), id);
			return deviceRepos.findByService(service).stream().map(d -> d.getAddressable()).collect(Collectors.toSet());
		} catch (NotFoundException nE) {
			throw nE;
		} catch (Exception e) {
			logger.error("Error getting service:  " + e.getMessage());
	 ...
	 * Return a set (set versus list to insure element uniqueness) of
	 * addressables that are associated to the devices of a device service. The
	 * device service is identified by name. Returns NotFoundException (HTTP
	 * 404) if the device service is not found by id. Returns ServiceException
	 * (HTTP 503) for unknown or unanticipated issues.
	 * 
	 * @param name
	 *            - unique name for the device service
	 * @return - set of unique addressable objects associated to devices of the
	 *         owning device service
	 */
	...
	/**
	 * Add a new DeviceService - name must be unique. Returns ServiceException
	 * (HTTP 503) for unknown or unanticipated issues. Returns
	 * DataValidationException (HTTP 409) if an associated addressable (by id or
	 * name) is not found.
	 * 
	 * @param DeviceService
	 *            object
	...
	private void attachAssociated(DeviceService service) {
		try {
			...
			if (addressable == null)
				throw new DataValidationException("A device service must be associated to a known addressable.");
	...
	/**
	 * Update the last connected time of the device service by database
	 * generated identifier. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if the device
	 * service cannot be found by the identifier provided.
	 * 
	 * @param id
	 *            - database generated identifier for the device service
	 * @param time
	...
	public boolean updateLastConnected(@PathVariable String id, @PathVariable long time) {
		try {
			...
			if (deviceService == null) {
				logger.error("Request to update last connected time with non-existent device service:  " + id);
				throw new NotFoundException(DeviceService.class.toString(), id);
			}
	...
	/**
	 * Update the last connected time of the device service by unique name of
	 * the device service. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if the device
	...
	public boolean updateLastConnectedByName(@PathVariable String name, @PathVariable long time) {
		try {
			...
			if (deviceService == null) {
				logger.error("Request to update last connected time with non-existent device service:  " + name);
				throw new NotFoundException(DeviceService.class.toString(), name);
			}
			return updateLastConnected(deviceService, time);
		} catch (NotFoundException nE) {
	...
	/**
	 * Update the last reported time of the device service by database generated
	...
	public boolean updateLastReported(@PathVariable String id, @PathVariable long time) {
		try {
			DeviceService deviceService = repos.findOne(id);
			if (deviceService == null) {
				logger.error("Request to update last reported time with non-existent device service:  " + id);
				throw new NotFoundException(DeviceService.class.toString(), id);
			}
	...
	/**
	 * Update the last reported time of the device service by unique name of the
	 * device service. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if the device
	 * service cannot be found by the name provided.
	 * 
	 * @param name
	...
	public boolean updateLastReportedByName(@PathVariable String name, @PathVariable long time) {
		try {
			DeviceService deviceService = repos.findByName(name);
			if (deviceService == null) {
				logger.error("Request to update last reported time with non-existent device service:  " + name);
				throw new NotFoundException(DeviceService.class.toString(), name);
			}
			return updateLastReported(deviceService, time);
		} catch (NotFoundException nE) {
	...
	/**
	 * Update the op state of the device service by database generated
	 * identifier. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if the device
	 * service cannot be found by the identifier provided.
	 * 
	 ...
	 *            - database generated identifier for the device service
	 * @param opState
	 *            - new op state for the device service (either enabled or
	 *            disabled)
	...
	/**
	 * Update the op status time of the device service by unique name of the
	 * device service. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if the device
	 * service cannot be found by the name provided.
	 * 
	...
	/**
	 * Update the admin state of the device service by database generated
	 * identifier. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if the device
	 * service cannot be found by the identifier provided.
	 * 
	 ...
	 *            - database generated identifier for the device service
	 * @param adminstate
	 *            - new admin state for the device service (either locked or
	 *            unlocked)
	 * @return - boolean indicating success of the operation
	 */
	...
	/**
	 * Update the admin state of the device service by device service name.
	 * Returns ServiceException (HTTP 503) for unknown or unanticipated issues.
	 * Returns NotFoundException (HTTP 404) if the device service cannot be
	 * found by the identifier provided.
	 * 
	 * @param name
	 *            - device service name
	 * @param opState
	...
	public boolean updateAdminStateByName(@PathVariable String name, @PathVariable String adminState) {
		try {
			DeviceService deviceService = repos.findByName(name);
			if (deviceService == null) {
				logger.error("Request to update admin state with non-existent device service name:  " + name);
				throw new NotFoundException(DeviceService.class.toString(), name);
			}
			return updateAdminState(deviceService, adminState);
		} catch (NotFoundException nE) {
	...
	/**
	 * Update the DeviceServcie identified by the id or name stored in the
	 * object provided. Id is used first, name is used second for identification
	 * purposes. Returns ServiceException (HTTP 503) for unknown or
	 * unanticipated issues. Returns NotFoundException (HTTP 404) if the device
	 * service cannot be found by the identifier provided.
	 * 
	 * @param deviceService2
	 *            - object holding the identifier and new values for the
	 *            DeviceService
	...
	public boolean update(@RequestBody DeviceService deviceService2) {
		try {
			...
			if (deviceService == null) {
				logger.error("Request to update with non-existent or unidentified device service (id/name):  "
	...
	private void updateDeviceService(DeviceService from, DeviceService to) {
		if (from.getAddressable() != null) {
			Addressable a = addressableDao.getByIdOrName(from.getAddressable());
			if (a != null)
				to.setAddressable(a);
			else {
				logger.error("Unable to locate addressable and cannot set device service addressable to null");
				if (from.getAddressable().getId() != null)
					throw new NotFoundException(Addressable.class.toString(), from.getAddressable().getId());
				else
					throw new NotFoundException(Addressable.class.toString(), from.getAddressable().getName());
			}
		}
	...
	/**
	 * Remove the DeviceService designated by database generated id. Returns
	 * ServiceException (HTTP 503) for unknown or unanticipated issues. Returns
	 ...
	 * 
	 * @param database
	 *            generated id for the device service
	...
	/**
	 * Remove the DeviceService designated by name. Returns ServiceException
	 * (HTTP 503) for unknown or unanticipated issues. Returns NotFoundException
