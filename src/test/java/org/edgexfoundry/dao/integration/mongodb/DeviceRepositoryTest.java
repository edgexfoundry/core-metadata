...
public class DeviceRepositoryTest {
	...
	public void testFindByServiceWithBadService() {
		...
		s.setId("abc");
		List<Device> devices = repos.findByService(s);
	...
	public void testFindByProfileWithBadProfile() {
		...
		p.setId("abc");
		List<Device> devices = repos.findByProfile(p);
	...
	public void testFindByAddressableWithBadAddressable() {
		Addressable a = AddressableData.newTestInstance();
		a.setId("abc");
		List<Device> devices = repos.findByAddressable(a);
	...
	public void testUpdate() {
		...
		assertEquals("Modified and created timestamps should be equal after creation", device.getModified(),
				device.getCreated());
		device.setDescription("new description");
		repos.save(device);
		// reread device
		Device device2 = repos.findOne(id);
		assertEquals("Device was not updated appropriately", "new description", device2.getDescription());
		assertNotEquals("after modification, modified timestamp still the same as the device's create timestamp",
