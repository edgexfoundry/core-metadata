...
public class DeviceServiceRepositoryTest {
	...
	/**
	 * Create and save an instance of the DeviceService before each test Note:
	...
	/**
	 * Clean up of the unit test Note: clean up also tests the delete operation
	 * of the repository
	 */
	@After
	...
	public void testFindAll() {
		List<DeviceService> as = repos.findAll();
	...
	public void testFindByAddressableWithBadAddressable() {
		Addressable a = AddressableData.newTestInstance();
		a.setId("abc");
		List<DeviceService> services = repos.findByAddressable(a);
	...
	public void testUpdate() {
		...
		assertEquals("Modified and created timestamps should be equal after creation", service.getModified(),
				service.getCreated());
		service.setDescription("new description");
		repos.save(service);
		// reread device
		DeviceService service2 = repos.findOne(id);
		assertEquals("Device service was not updated appropriately", "new description", service2.getDescription());
		assertNotEquals(
