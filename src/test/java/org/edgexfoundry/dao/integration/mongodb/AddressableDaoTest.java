...
public class AddressableDaoTest {
	...
	public void testGetById() {
		Addressable a = dao.getById(addressableId);
		assertNotNull("Addressable is null on getById", a);
	...
	public void testGetByName() {
		Addressable a = dao.getByName(TEST_ADDR_NAME);
		assertNotNull("Addressable is null on getByName", a);
		checkTestData(a, addressableId);
	}
	...
	public void testGetByIdOrName() {
		Addressable a = newTestInstance();
		assertNotNull("Addressable is null on getByIdOrName with valid id", dao.getByIdOrName(a));
		a.setName(null);
		a.setId(addressableId);
		assertNotNull("Addressable is null on getByIdOrName with valid name", dao.getByIdOrName(a));
	}

	@Test
	public void testGetByIdOrNameWithNull() {
		assertNull("No addressable should be found with null on getByIdOrName", dao.getByIdOrName(null));
	}
	...
	public void testGetByIdOrNameWithBadIdentifiers() {
		Addressable a = newTestInstance();
		a.setId("badid");
		assertNull("No addressable should be found with bad id on getByIdOrName", dao.getByIdOrName(a));
		a.setId(null);
		a.setName("badname");
		assertNull("No addressable should be found with bad id on getByIdOrName", dao.getByIdOrName(a));
	}
	...
	public void testIsAddressableAssociatedToDevice() {
		...
		assertTrue("Found no associated Devices and should have", dao.isAddressableAssociatedToDevice(a));
	}
	...
	public void testGetOwningServices() {
		...
		deviceRepos.save(device);
		List<Asset> services = dao.getOwningServices(a);
		assertEquals("Did not find correct number of associated DeviceServices ", 1, services.size());
		assertEquals("Associated service is not the same", service.getId(), ((DeviceService) services.get(0)).getId());
	}
