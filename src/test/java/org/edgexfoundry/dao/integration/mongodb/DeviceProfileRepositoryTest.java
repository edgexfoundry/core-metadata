...
public class DeviceProfileRepositoryTest {
	...
	public void testFindAll() {
		List<DeviceProfile> as = repos.findAll();
	...
	public void testFindByManufacturerWithBadManufacturer() {
		...
		assertTrue("Find by manufacturer returns device profile with bad manufacturer", profiles.isEmpty());
	}
	...
	public void testUpdate() {
		...
		assertEquals("Modified and created timestamps should be equal after creation", p.getModified(), p.getCreated());
		p.setDescription("new description");
		repos.save(p);
		// reread device profile
		DeviceProfile p2 = repos.findOne(id);
		assertEquals("Device profile was not updated appropriately", "new description", p2.getDescription());
		assertNotEquals(
				"after modification, modified timestamp still the same as the device profile's create timestamp",
				p2.getModified(), p2.getCreated());
	}
