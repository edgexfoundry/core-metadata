...
public class DeviceReportRepositoryTest {
	...
	/**
	 * Create and save an instance of the DeviceProfile before each test Note:
	...
	public void testFindAll() {
		List<DeviceReport> as = repos.findAll();
	...
	public void testUpdate() {
		...
		assertEquals("Modified and created timestamps should be equal after creation", r.getModified(), r.getCreated());
		String[] exptds = { "vD3" };
		r.setExpected(exptds);
		repos.save(r);
		// reread device report
		DeviceReport r2 = repos.findOne(id);
		assertArrayEquals("Device report was not updated appropriately", exptds, r2.getExpected());
		assertNotEquals("after modification, modified timestamp still the same as the device report's create timestamp",
				r2.getModified(), r2.getCreated());
	}

	@Test
	public void testDelete() {
		DeviceReport p = repos.findOne(id);
		repos.delete(p);
