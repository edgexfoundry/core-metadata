...
public class ScheduleEventRepositoryTest {
	...
	public void testFindAll() {
		List<ScheduleEvent> as = repos.findAll();
...
//	@Test
	...
	public void testScheduleEventWithSameName() {
		ScheduleEvent event = ScheduleEventData.newTestInstance();
		Addressable a = addrRepos.findByName(AddressableData.TEST_ADDR_NAME);
		event.setAddressable(a);
		repos.save(event);
	...
	public void testUpdate() {
		...
		assertEquals("Modified and created timestamps should be equal after creation", event.getModified(),
				event.getCreated());
		event.setParameters("{'test':'data'}");
		repos.save(event);
		// reread Schedule event
		ScheduleEvent e2 = repos.findOne(id);
		assertEquals("Schedule event was not updated appropriately", "{'test':'data'}", e2.getParameters());
		assertNotEquals(
	...
	public void testDelete() {
		ScheduleEvent p = repos.findOne(id);
		repos.delete(p);
